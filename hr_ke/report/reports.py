    ...
    def GetNSSFReturns(self):
        for rec in self:
            if rec.slip_ids:
                ...
                # paid workers
                ws['B1'] = '01'
                ws['A2'] = 'EMPLOYER TRA PIN'
                # Employer TRA PIN in the HR Settings
                ws['B2'] = rec.journal_id.company_id.employer_tra or None
                ...
                ws['D' + str(fr - 1)] = 'ID NO'
                ws['E' + str(fr - 1)] = 'MEMBER TRA PIN NO'
        ...
        for rec in self:
            ...
            for slip in rec.slip_ids:
                ...
                if not slip.employee_id.disability:
                    if not slip.employee_id.tax_pin:
                        raise ValidationError(
                            _('TRA PIN Number for %s is missing!' % slip.employee_id.name))
                    data_employee = [
                        slip.employee_id.tax_pin,  # A
                ...
                if slip.employee_id.disability:
                    data_disabled = [
                        slip.employee_id.tax_pin,  # A TRA PIN for Employee
