...
class CarRepairSupport(http.Controller):
    ...
    @http.route(['/car_repair_maintenance_service/request_submitted'], type='http', auth="public", methods=['POST'],
                website=True)
    def request_submitted(self, **post):
        ...
        if Partner:
            ...
            if len(attachment_list) > 0:
                group_msg = _(
                    'Customer has sent %s attachments to this car repair ticket. Name of attachments are: ') % (
                                len(attachment_list))
...
class website_account(website_account):
    ...
    def _prepare_portal_layout_values(self):  # odoo11
        ...
        partner = request.env.user.partner_id
        values.update({
            'repair_request_count': request.env['car.repair.support'].sudo().search_count(
    ...
    @http.route(['/my/repair_request/<model("car.repair.support"):repair_request>'], type='http', auth="user",
                website=True)
    def my_repair_request(self, repair_request=None, **kw):
        ...
        if len(attachment_list) > 0:
            group_msg = _('Customer has sent %s attachments to this Car repair ticket. Name of attachments are: ') % (
                len(attachment_list))
            for attach in attachment_list:
                group_msg = group_msg + '\n' + attach.filename
            group_msg = group_msg + '\n' + '. You can see top attachment menu to download attachments.'
        ...
        return request.render("car_repair_maintenance_service.display_car_repair_request_from",
                              {'repair_request': repair_request, 'user': request.env.user})
