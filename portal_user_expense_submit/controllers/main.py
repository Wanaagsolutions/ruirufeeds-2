...
class CustomerPortal(CustomerPortal):
    ...
    def expense(self, redirect=None, date=None, **post):
        ...
        product_name = http.request.env['product.product'].sudo().search([('can_be_expensed', '=', True),('company_id', '=', http.request.env.company.id)])
        unit_price = http.request.env['hr.expense'].sudo().search([('company_id', '=', http.request.env.company.id)])
        employee_name = http.request.env['hr.employee'].sudo().search([('company_id', '=', http.request.env.company.id)])
        paid_name = http.request.env['hr.expense'].sudo().search([('company_id', '=', http.request.env.company.id)])
        date = fields.Date.today()
        currency = http.request.env.user.partner_id.company_id.currency_id.symbol
        product_uom_id = http.request.env['uom.uom'].sudo().search([])
        analytic_account_id = http.request.env['account.analytic.account'].sudo().search([('company_id', '=', http.request.env.company.id)])
    ...
    def confirm_form(self, **POST):
        ...
        # expense_id._create_sheet_from_expenses()
    ...
    def my_expense(self, order=None, access_token=None, **kw):
        values = {}
        expense_form = http.request.env['hr.expense'].sudo().browse([order])

        values.update({
            'page_name': 'form_expense',
            'expense_form': expense_form,
            'sheet_id': expense_form['sheet_id']
        })
