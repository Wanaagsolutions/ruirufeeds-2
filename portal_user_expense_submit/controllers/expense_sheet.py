...
class CustomerPortal(CustomerPortal):
    ...
    @http.route(['/submit/expense_sheet/<int:order>'], type='http', auth="public", website=True)
    def submit_expense_sheet(self,order):
       
        expense_sheet = http.request.env['hr.expense.sheet'].sudo().search(
            [("id", "=", order)])
        expense_sheet.action_submit_sheet()
        return werkzeug.utils.redirect('/expence_sheet/list/expenses')
